...
class Example(Frame):
    ...
    # фильтр "контур" - image.filter(ImageFilter.CONTOUR). После чего мы измененное изображение выводим на ранее
    # созданную нами область (полотно) - self.display.itemconfigure(self.display_img, image=self.photo, anchor="nw").
    def contour(self):
    ...
    def create_menu(self):
        ...
        # подпрограмме contour(), о которой мы говорили выше.
        self.btn_contour = Button(text="Контур", height=2, width=12, command=self.contour)
        self.btn_contour.place(x=25, y=120)

        self.btn_contour = Button(text="Тиснение", height=2, width=12, command=self.emboss)
        self.btn_contour.place(x=25, y=180)


if __name__ == '__main__':
    root = Tk()
    root.title("Пример 4 - Применение фильтра CONTOUR")
